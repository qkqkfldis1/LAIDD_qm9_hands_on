...
def imp_exp_attn(implicit_attn, explicit_edge):
    ...
    def func(edges): # implicit_attn: score, explicit_edge: edge_features, proj_e
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, g, h, e):
        ...
        Q_h = self.Q(h) # [2358, 64]
        K_h = self.K(h) # [2358, 64]
        V_h = self.V(h) # [2358, 64]
        proj_e = self.proj_e(e) # [4888, 64]
...
class GraphTransformerEdgeNet(nn.Module):
    ...
    def forward(self, g):
        ...
        e = g.edata['f'] # [4888, 4]
        # input embedding
        h = self.embedding_h(h) # [2358, 64]
        h = self.in_feat_dropout(h)
        e = self.embedding_e(e) # [4888, 64]
        # print('edge', e.shape)
