...
from qm9_dataset import QM9DGLDataset, collate
from torch.utils.data import DataLoader
from omegaconf import OmegaConf
from graph_transformer import GraphTransformerNet
from graph_transformer_edge import GraphTransformerEdgeNet
...
def train_epoch(epoch, model, loss_fnc, dataloader, optimizer, scheduler, FLAGS, device):
    ...
    for i, (g, y) in enumerate(tqdm(dataloader, total=len(dataloader))):
        g = g.to(device)
        y = y.to(device)
...
parser.add_argument('--config', type=str, default='./configs/graph_transformer.yaml', help='configration for model')
...
################ 2. Prepare model
if FLAGS.model == 'GraphTransformer':
    model = GraphTransformerNet(FLAGS.net_params)
    print('GraphTransformer created')
elif FLAGS.model == 'GraphTransformerEdge':
    model = GraphTransformerEdgeNet(FLAGS.net_params)
    print('GraphTransformerEdgeNet created')
