...
class QM9DGLDataset(Dataset):
    ...
    def load_data(self):
        ...
        data = data[self.mode]
        data_csv = pd.read_csv('./data/qm9.csv')

        molId_to_smile = {}
        for mol_id, smile in zip(data_csv['mol_id'], data_csv['smiles']):
            molId_to_smile[mol_id] = smile

        self.molId_to_smile = molId_to_smile
    ...
    def __getitem__(self, idx):
        ...
        smiles = self.molId_to_smile[mol_id]
...
def collate(samples):
    graphs, y, _ = map(list, zip(*samples))
    batched_graph = dgl.batch(graphs)
    return batched_graph, torch.tensor(y)


def collate_multimodal(samples):
    graphs, y, smiles = map(list, zip(*samples))
    batched_graph = dgl.batch(graphs)
    return batched_graph, torch.tensor(y), smiles
